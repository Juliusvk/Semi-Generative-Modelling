...
if mod(n_S,k) ~= 0
    error('k-fold CV not possible with this training set size')
else
    fold_size = n_S/k;
end
...
for i=1:k
    ...
    S_val = S_S((i-1)*fold_size+1 : i*fold_size,1);
    G_val = G_S((i-1)*fold_size+1 : i*fold_size,1);
    L_val = L_S((i-1)*fold_size+1 : i*fold_size,1);
    C_val = C_S((i-1)*fold_size+1 : i*fold_size,1);
    F_val = F_S((i-1)*fold_size+1 : i*fold_size,1);
    S_train((i-1)*fold_size+1 : i*fold_size) = [];
    G_train((i-1)*fold_size+1 : i*fold_size) = [];
    L_train((i-1)*fold_size+1 : i*fold_size) = [];
    C_train((i-1)*fold_size+1 : i*fold_size) = [];
    F_train((i-1)*fold_size+1 : i*fold_size) = [];
